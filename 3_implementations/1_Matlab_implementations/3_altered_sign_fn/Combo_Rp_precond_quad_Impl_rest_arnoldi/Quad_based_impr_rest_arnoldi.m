function [f, iter, fm] = Quad_based_impr_rest_arnoldi(A, v, V, H, eta, c_norm, theta, m, m1, m2, max_iter, thick_num, tol, min_decay)
    ...
    for k = 1:max_iter
        ...
        if k > 1
            if k <= m2
                [v, H, V_big,eta] = right_precondi_Arnoldi_process(A, m1, m+ell, ell+1, V_big, H, theta);
            else
        ...
        if k == 1 % in the first iteration g_1 = f and we use the "closed" form
            ...
            %         l2
            [h2, l] = Quadrature_rul_invsqrt(A, active_nodes, subdiag, thick_replaced, H, m, tol, ell, k, l);
