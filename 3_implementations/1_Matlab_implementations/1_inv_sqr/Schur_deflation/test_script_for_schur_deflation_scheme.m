...
% Compute f(A)x using Schur deflation scheme
result_schur = Schur_deflation_scheme(A, x, m, k);

finish = cputime;
disp(['Time taken by schur deflation scheme = ', num2str(finish - start), ' s']);
...
rel_err = norm(exact_result - result_schur)/norm(result_schur);
...
if rel_err < tol
    disp('Schur deflation method verified.');
else
    disp('Schur deflation not verified.');
end